...
// Probe 2: full BK=64 tile, B N-major (trans_b=1), 4 wgmma k-steps.
// Block stride along N is fixed at the value probe 1 found (enc 512 = 8192B); sweep the
// per-k16-step descriptor advance.
#include <cuda.h>
#include <cuda_fp16.h>
...
static constexpr int M=64,N=128,K=64,SMEM=200704;
...
__global__ __launch_bounds__(128) void probe(const __grid_constant__ CUtensorMap ta,
    const __grid_constant__ CUtensorMap tb,float*C,uint32_t kadv){
  ...
  for(int ks=0;ks<4;ks++){
    uint64_t da=mkdesc(su(sA)+ks*32,1,64,1);
    uint64_t db=mkdesc(su(sB)+ks*kadv,512,64,1);
...
int main(){
  ...
  for(int m=0;m<M;m++)for(int n=0;n<N;n++){float s=0;
    for(int k=0;k<K;k++) s+=__half2float(hA[m*K+k])*__half2float(hB[k*N+n]); ref[m*N+n]=s;}
  half*dA,*dB; float*dC;
  ...
  const uint32_t cand[]={16,32,64,128,256,512,1024,2048,4096};
  for(uint32_t k:cand){
    CK(cudaMemset(dC,0,M*N*4));
    probe<<<1,128,SMEM>>>(ta,tb,dC,k);
    if(cudaGetLastError()!=cudaSuccess||cudaDeviceSynchronize()!=cudaSuccess){printf("fault kadv=%u\n",k);continue;}
    CK(cudaMemcpy(hC.data(),dC,M*N*4,cudaMemcpyDeviceToHost));
    double e=0; for(int i=0;i<M*N;i++) e=fmax(e,fabs(hC[i]-ref[i]));
    printf("  kadv=%-6u bytes  maxerr=%.0f %s\n",k,e,e==0.0?"  <== MATCH":"");
  }
  return 0;
}
